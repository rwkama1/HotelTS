...
import { PassengerController } from "../../passenger_maintenance/PassengerController";
...
export default class LRegisterHotelReservation
 {
    ...
    registerPassenger=async(dtopassenger:DTOPassenger)=>
    {
        ...
        if(lreservation.passenger===null)
        {
            ...
            const result=await LCUDPassenger.registerPassenger(dtopassenger);
            if(result===true)
            {
                let enterp=lreservation.enterPassenger(dtopassenger.idcard);
                return enterp
            }
        }
        else
