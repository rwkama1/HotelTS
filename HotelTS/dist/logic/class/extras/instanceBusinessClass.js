...
class InstanceLogicClass {
    static instanceLUser = (dtouser) => {
        var logicuser = null;
        if (dtouser != null) {
            logicuser = new LUser_1.default(dtouser.idcard, dtouser.name, dtouser.surname, dtouser.address, dtouser.phone, dtouser.typeuserr, dtouser.password, dtouser.hashh, dtouser.maill, dtouser.statee);
        }
        return logicuser;
    };
    static instanceLPassenger = (dtopassenger) => {
        ...
        if (dtopassenger != null) {
            logicpassenger = new LPassenger_1.default(dtopassenger.idcard, dtopassenger.name, dtopassenger.surname, dtopassenger.country, dtopassenger.town, dtopassenger.address, dtopassenger.phone, dtopassenger.maill, dtopassenger.salt, dtopassenger.password, dtopassenger.statee);
        }
    ...
    static instanceLRoom = (dtoroom) => {
        var logicroom = null;
        if (dtoroom != null) {
            logicroom = new LRoom_1.default(dtoroom.numberroom, dtoroom.typeroom, dtoroom.typebed, dtoroom.acommodation, dtoroom.description, dtoroom.value, dtoroom.statee, dtoroom.image);
        }
        return logicroom;
    };
